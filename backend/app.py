...
@app.before_request
def payment_gate():
    if (request.path in {"/create-payment", "/midtrans-webhook"} or request.path.startswith("/download/")) and os.getenv("PAYMENTS_ENABLED", "false").lower() != "true":
        return jsonify(success=False, message="Pembayaran belum diaktifkan."), 503
